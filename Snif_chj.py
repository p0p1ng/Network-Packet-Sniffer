    ...
    def run(self):
        ...
        for stamp, pkt in self.CSniffer.pc:
            if not self._isStop:
                ...
                if time.time() - start_time >= 1 or index % 20 == 0:  # index
                    start_time = time.time()
...
class Dlg(QDialog, Ui_Dialog):  # 次界面
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowTitle('Net-Packet-Sniffer')
        # self.setWindowIcon(QIcon('img/dog.png'))弃用，写到下面了。而且相对路径不太靠谱。
        
        self.timer = QTimer(self)  # 创建定时器
        self.timer.timeout.connect(self.refresh_ui)  # 连接定时器的超时信号和槽函数
        ...
        self.pro_color = {'TCP': QColor(135, 206, 235, 50), 'UDP': QColor(0, 255, 0, 50),
                          'HTTP': QColor(255, 215, 0, 50), 'ICMP': QColor(255, 97, 0, 50), 'IGMP': QColor(156, 102, 31, 50),
        ...
        self.model.setHorizontalHeaderLabels(
            ['序号', '时间戳', '源地址', '目标地址', '协议类型', '信息'])  # 设置水平方向四个头标签文本内容
    ...
    def refresh_ui(self):
        # 在定时器触发时调用该方法，用于刷新界面
        self.tableView.repaint()  # 刷新tableView界面
        print("refresh_ui")
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QIcon('/Lab1-Sniffer/img/dog1.png'))
    win = MainWin()
    win.show()
    sys.exit(app.exec())
