...
class CSniffer:
    ...
    def set_device(self, name, ts=50, ts_in_ns=False):
        ...
        #测试用，等会删 pc = pcap.pcap("en0", timeout_ms=50, timestamp_in_ns=False)
    ...
    def format_packet(self, timestamp, pkt):  #timestamp  packet 
                ...
                if 1 == 1:
                    ...
                    if not R:
                        ...
                        try:
                            ...
                            response_list = ['protocol version: '+pro_ver, 'status code: '+status_code, 'response: '+str(response)]
        ...
        try:
            ...
            Frames['空帧'] = ['硬件类型: '+'hard_type', '协议类型: '+'pro_type', '硬件地址长度: '+'hard_len', '协议地址长度: '+'pro_len', \
                              '操作类型: '+'op', '发送者硬件地址: '+'send_mac', '发送者IP地址: '+'src_ip', '目标硬件地址: '+'dst_mac', '目标IP地址: '+'dst_ip']
            attach_info = " ".join(Frames['空帧'])
            protocol = 'ARP'
            return [tmstmp, '0.0.0.0', '0.0.0.0', protocol, attach_info, hex_rep, Frames]#data_format
